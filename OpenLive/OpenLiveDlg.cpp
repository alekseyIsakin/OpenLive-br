...
LRESULT COpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
	...
	lpAgoraObject->SetToken(netToken.GetHostToken());
	...
	lpAgoraObject->SetClientRole(CLIENT_ROLE_BROADCASTER);
	lpAgoraObject->JoinChannel(netToken.GetHostName(), 0, token.length() > 0 ? token.c_str() : NULL);
